    ...
    private static void putOneShipOnBoard(Ocean ocean, Ship ship) {
        if (ship.getIsVertical()) {
            for (int index = 0; index < ship.getShipLength(); index++) {
                ocean.getSquares().get(ship.getCoordinateY() + index).set(ship.getCoordinateX(),
                        ship.getSqareByIndex(index));
            }
        } else {
            for (int index = 0; index < ship.getShipLength(); index++) {
                ocean.getSquares().get(ship.getCoordinateY()).set(ship.getCoordinateX() + index,
                        ship.getSqareByIndex(index));
            }
        }
    }
